...
describe('CacheStrategyManager', () => {
  ...
  describe('CACHE_STRATEGIES', () => {
    ...
    it('falls back to default when strategy entry is undefined', () => {
      const original = { ...CACHE_STRATEGIES };
      // Force undefined entries to exercise nullish coalescing branches
      (CACHE_STRATEGIES as any).monitoring_stats = undefined;
      (CACHE_STRATEGIES as any).project_history = undefined;
      (CACHE_STRATEGIES as any).user_projects = undefined;
      (CACHE_STRATEGIES as any).user_data = undefined;
      (CACHE_STRATEGIES as any).ssl_stats = undefined;

      expect(CacheStrategyManager.getStrategy('monitoring_stats')).toEqual({
        priority: CachePriority.MEDIUM,
        ttl: 5 * 60 * 1000,
        invalidateOnWrite: true,
        invalidatePatterns: [],
      });
      expect(CacheStrategyManager.getStrategy('project_history_123')).toEqual({
        priority: CachePriority.MEDIUM,
        ttl: 5 * 60 * 1000,
        invalidateOnWrite: true,
        invalidatePatterns: [],
      });
      expect(CacheStrategyManager.getStrategy('user_projects_123')).toEqual({
        priority: CachePriority.MEDIUM,
        ttl: 5 * 60 * 1000,
        invalidateOnWrite: true,
        invalidatePatterns: [],
      });
      expect(CacheStrategyManager.getStrategy('user_123')).toEqual({
        priority: CachePriority.MEDIUM,
        ttl: 5 * 60 * 1000,
        invalidateOnWrite: true,
        invalidatePatterns: [],
      });
      expect(CacheStrategyManager.getStrategy('ssl_stats')).toEqual({
        priority: CachePriority.MEDIUM,
        ttl: 5 * 60 * 1000,
        invalidateOnWrite: true,
        invalidatePatterns: [],
      ...
      // restore
      Object.assign(CACHE_STRATEGIES as any, original);
    });
  });
});
