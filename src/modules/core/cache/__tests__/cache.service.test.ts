...
import { CachePriority } from '@/modules/core/cache/cache.contract';
...
describe('CacheService', () => {
  ...
  describe('has', () => {
    ...
    it('returns false and deletes when expired', () => {
      const now = Date.now();
      jest
        .spyOn(Date, 'now')
        .mockReturnValueOnce(now) // set
        .mockReturnValueOnce(now + 1000); // has after expired

      cacheService.set('k', 'v', 10);
      expect(cacheService.has('k')).toBe(false);
  ...
  describe('TTL and expiration', () => {
    ...
    it('expires and deletes entry on get after TTL', () => {
      const now = Date.now();
      jest
        .spyOn(Date, 'now')
        .mockReturnValueOnce(now) // set
        .mockReturnValueOnce(now) // get before expire
        .mockReturnValueOnce(now + 1000); // get after expire

      cacheService.set('exp', 'v', 100);
      expect(cacheService.get('exp')).toBe('v');
      expect(cacheService.get('exp')).toBeNull();
  ...
  describe('cache invalidation on write', () => {
    it('should invalidate related patterns', () => {
      ...
      if (strategy.invalidateOnWrite) {
        strategy.invalidatePatterns.forEach((pattern: string) => {
